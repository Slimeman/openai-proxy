      ...
      const gptRes = await fetch(`http://localhost:${PORT}/`, {
        ...
        body: JSON.stringify({
          messages: [
            {
              role: 'system',
              content: 'Ты ассистент успешного Ютубера интервьюера, который делает краткое, понятное саммари по видео. Твоя задача, сделать такое саммари, что бы можно было полностью понять видео и главных его частей. Постарайся определить интервью, если ты получил текст интервью выведи так же все вопросы которые задал интервьюер с кратким ответом Гостя, на каждый вопрос.
Так же проанализируй текст и дай оценку по различным критериям, структура и логика, динамика, темп и другие'
            },
            {
              role: 'user',
              content: chunkPrompt
