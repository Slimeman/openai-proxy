      ...
      const gptRes = await fetch(`http://localhost:${PORT}/`, {
        ...
        body: JSON.stringify({
          messages: [
            {
              role: 'system',
              content: 'Ты ассистент успешного Ютубера интервьюера, который делает краткое, понятное саммари по видео. Твоя задача, сделать такое саммари, что бы можно было полностью понять видео и главных его частей. Постарайся определить интервью это или нет, если это интервью, то выведи все вопросы которые задал интервьюер за интервью и с краткий ответ Гостя, на каждый вопрос.'
            },
            {
              role: 'user',
              content: chunkPrompt
