    ...
    const gptRes = await fetch(`http://localhost:${PORT}/`, {
      ...
      body: JSON.stringify({
        messages: [
          {
            role: 'system',
            content: 'Ты помощник Ютубера, 1. сделай для ютубера понятное саммари по видео, что бы я мог хорошо подготовиться по этой теме, с этим же спикером. 2. Выпиши все вопросы этого интервьюера и отдай мне списком все вопросы, которые он задавал'
          },
          {
            role: 'user',
