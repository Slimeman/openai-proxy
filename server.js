...
// 💰 Оценка дохода (реалистичная, калиброванная на твой кейс)
function estimateRevenueRange(views, language, duration, categoryId) {
  let baseRPM = 0.0038; // базовый RPM = $3.8 за 1000 просмотров

  // Язык
  if (language && typeof language === 'string') {
    const lang = language.toLowerCase();
    if (lang.startsWith('en')) baseRPM += 0.002; // boost для англ.
  }

  // Длительность
  if (duration >= 10 && duration < 30) baseRPM += 0.001;
  else if (duration >= 30 && duration < 60) baseRPM += 0.002;
  ...
  // Итоговая граница дохода с понижающим множителем
  const rpmLow = Math.max(baseRPM * 0.9, 0.0025);
  const rpmHigh = baseRPM * 1.2;

  // Умножаем на 0.5 — примерно половина просмотров монетизируется
  return [views * rpmLow * 0.5, views * rpmHigh * 0.5];
}
